...
const stepsKeys = [
    "auth.signUpSteps.account",
    "auth.signUpSteps.plan",
    "auth.signUpSteps.payment"
]
...
const SignUpPage = ({
    ...
    const handleNext = () => {
        if (step === 0) {
            ...
            if (name.length < 6) {
                toast.error(t("auth.errors.nameLength"))
                return
            }
            ...
            >
                        ...
                        stepsKeys.map((key, i) => {
                            ...
                            return (
                                <div key={key} className="flex items-center gap-2">
                                    <div
                                        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-colors ${i <= step
                                                    ? "bg-white text-black"
                ...
                <div className="flex justify-between items-center mt-4">
                    ...
                    }
                    {
                        ...
                        <button
                            onClick={handleNext}
                            className="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-sm text-sm font-semibold cursor-pointer hover:opacity-80"
                        >
                            {t("auth.next")} <ChevronRight size={14} />
                        </button>
                    }
                </div>
